...
export default function LocaleSwitcherSelect({
  ...
  return (
    <div className="relative inline-block">
      ...
      <button
        aria-label={label}
        className={`flex items-center justify-between rounded-sm p-2 border transition-colors hover:bg-slate-200 ${isPending ? 'pointer-events-none opacity-60' : ''
          }`}
        ...
        <div className="absolute mt-2 w-full min-w-[8rem] bg-white shadow-md rounded-sm border">
          <ul className="py-1">
            {items.map((item) => (
              <li
                key={item.value}
                className={`flex px-3 py-2 cursor-pointer hover:bg-slate-100 gap-2 ${item.value === selectedValue ? 'font-bold text-blue-600' : 'text-gray-900'
                  }`}
